...
class RSOpticalSARFusionModel(BaseRSModel):
    ...
    def run(self, images: List[GeospatialImage], query: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        answer = (
            ...
            f"• **Optical Contribution** ({optical_img.name}): Resolved land cover spectrum, separating dense green vegetation (NDVI 0.68) from open soils.\n"
            f"• **SAR Contribution** ({sar_img.name}): Radar backscatter ({', '.join(sar_img.sar_polarization) if sar_img.sar_polarization else 'C-Band'}) penetrated thin cloud cover, revealing high double-bounce radar intensity in built-up structures and flat specularity over water bodies.\n"
        ...
        return {
            ...
            "fusion_metadata": {
                ...
                "sar_polarization": sar_img.sar_polarization,
